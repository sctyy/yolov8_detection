...
class AutoBackend(nn.Module):
    ...
    def postprocess(preds, im, orig_imgs, conf_thresh=0.25, iou_thresh=0.45, max_det=100, classes=None):
        ...
        for pred, orig_img in zip(preds, orig_imgs):
            # pred的结构假设为：[x1, y1, x2, y2, conf, class_prob, ...]，其中...表示分割掩码数据

            # 1. 非极大值抑制（NMS）
            boxes = pred[:, :4]
            scores = pred[:, 4]
            indices = torchvision.ops.nms(boxes, scores, iou_thresh)
            indices = indices[:max_det]  # 限制最大检测数量

            # 2. 置信度筛选
            indices = indices[scores[indices] > conf_thresh]

            # 3. 类别筛选（如果指定了classes）
            if classes is not None:
                class_filter = pred[indices, 5].isin(classes)
                indices = indices[class_filter]

            # 4. 边界框缩放回原始图像尺寸
            scale_factor = torch.tensor([orig_img.width / im.width, orig_img.height / im.height,
                                         orig_img.width / im.width, orig_img.height / im.height])
            boxes[indices] *= scale_factor

            # 5. 分割掩码处理（假设分割掩码在preds的后续列中）
            if pred.shape[1] > 6:  # 假设分割掩码从第7列开始
                masks = pred[indices, 6:]
                masks = F.interpolate(masks.unsqueeze(0), size=orig_img.size, mode='bilinear', align_corners=False)
                masks = masks.squeeze(0)
            else:
                masks = None

            # 6. 类别识别
            classes = pred[indices, 5].int().tolist()
