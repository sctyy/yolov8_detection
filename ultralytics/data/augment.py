...
class LetterBox:
    ...
    def __init__(self, new_shape=(640, 640), auto=False, scaleFill=False, scaleup=True, center=True, stride=32):
        ...
        Examples:
            >>> letterbox = LetterBox(new_shape=(640, 640), auto=False, scaleFill=True, scaleup=True, stride=32)
...
class SaveStep:
    def __init__(self, transform, save_dir, step_name):
    ...
    def __call__(self, labels):
        img_in = labels['img'].copy()
        cv2.imwrite(str(self.save_dir / f"{self.step_name}_{self.cnt:06d}_in.jpg"), img_in)

        labels_out = self.transform(labels)

        img_out = labels_out['img'].copy()
        cv2.imwrite(str(self.save_dir / f"{self.step_name}_{self.cnt:06d}_out.jpg"), img_out)
        self.cnt += 1
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    # mosaic = SaveStep(mosaic, "/mnt/d/疵点数据库/标注疵点/mosaic4", "mosaic")
    ...
    # affine = SaveStep(affine, "/mnt/d/疵点数据库/标注疵点/affine4", "affine")
